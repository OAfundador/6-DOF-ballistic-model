...
class BallisticSimulator:
    ...
    def simulate(self, max_time=100.0, alpha0_deg=0.0, beta0_deg=0.0,
                ...
                stop_on_fuze=False, fuze_target_center=None,
                fuze_radius_m=24.38, fuze_arm_time_s=0.5):
        """
        ...
        if stop_on_fuze:
            if fuze_target_center is None:
                raise ValueError("fuze_target_center deve ser informado quando stop_on_fuze=True")

            centro_fuze = np.array(fuze_target_center, dtype=float)

            def fuze_event(t, y):
                if t < fuze_arm_time_s:
                    return 1.0
                pos = np.array([y[9], y[10], y[11]], dtype=float)
                return float(np.linalg.norm(pos - centro_fuze) - fuze_radius_m)

            fuze_event.direction = -1
            fuze_event.terminal = True
            events.append(fuze_event)
        ...
        if len(sol.t_events) > 0 and len(sol.t_events[0]) > 0:
            stop_reason = "ground"
        if stop_on_fuze and len(sol.t_events) > 1 and len(sol.t_events[1]) > 0:
            stop_reason = "fuze"
        self.stop_reason = stop_reason
        self.fuze_event_enabled = stop_on_fuze
        
        if sol.success:
            print(f"✓ Integração bem-sucedida!")
            print(f"  Tempo de voo: {sol.t[-1]:.2f} s")
            print(f"  Motivo da parada: {stop_reason}")
        else:
...
class SimulationResult:
    ...
    def __init__(self, solution, simulator):
        ...
        self.stop_reason = getattr(simulator, "stop_reason", "unknown")
        self.fuze_event_enabled = getattr(simulator, "fuze_event_enabled", False)
...
# =============================================================================
# MAIN CANONICA DO ARTIGO
# =============================================================================
#
# A proposta deste arquivo e manter o motor 6-DOF original em um unico script
# verificavel. Novas camadas de pesquisa entram como apendices importados aqui,
# sem alterar as classes do motor acima.
#
# Para rodar outro cenario, edite apenas os parametros dentro de
# run_article_verification_main().
# =============================================================================

def run_article_verification_main():
    """
    Executa um tiro 6-DOF e avalia o fuze/proximidade + dano fragmentario.

    Este e o ponto de verificacao principal do projeto:
    1. integra a trajetoria pelo motor legado;
    2. escolhe o ponto de burst com o fuze de proximidade;
    3. avalia o dano no Shahed usando o modelo modular importado.
    """
    ...
    from apendice_dano import (
        avaliar_fuze_e_dano,
        criar_alvo_shahed,
        criar_ogiva_vt_fcl_mk49,
    )

    # -------------------------------------------------------------------------
    # CENARIO EDITAVEL
    # -------------------------------------------------------------------------
    aero_coefficients_path = project_root / "data" / "aero_coefficients_5in38.xlsx"

    projectile_name = 'Projetil Naval 5"/38 AA VT(FCL)'
    muzzle_velocity_mps = 807.0
    elevation_deg = 39.6
    azimuth_deg = -1.35

    alvo = criar_alvo_shahed(centro=(16673.0, 200.0, 0.7))
    ogiva = criar_ogiva_vt_fcl_mk49()
    raio_fuze_m = 24.38
    tempo_armar_fuze_s = 0.5
    parar_simulacao_no_fuze = True
    ...
    def formatar_vetor(v):
        return f"({v[0]:.3f}, {v[1]:.3f}, {v[2]:.3f})"

    # -------------------------------------------------------------------------
    # MOTOR 6-DOF ORIGINAL
    # -------------------------------------------------------------------------
    print("\n" + "=" * 80)
    print("VERIFICACAO CANONICA: 6-DOF + FUZE + DANO FRAGMENTARIO")
    print("=" * 80)
    print(f"Coeficientes aerodinamicos: {aero_coefficients_path}")
    print(f"Tiro: elevacao={elevation_deg:.2f} deg | azimute={azimuth_deg:.2f} deg")
    print(f"Alvo Shahed: centro={formatar_vetor(alvo['centro'])} m")
    print(f"Orientacao alvo: nariz={formatar_vetor(alvo['direcao_nariz'])}, normal={formatar_vetor(alvo['normal'])}")
    print(f"Fuze: raio={raio_fuze_m:.2f} m | armado apos {tempo_armar_fuze_s:.2f} s")
    print(f"Parar simulacao no fuze: {parar_simulacao_no_fuze}")
    print(f"Ogiva: {ogiva['nome']}")

    aero_coeffs = RealAerodynamicCoefficients(str(aero_coefficients_path))
    ...
    result = simulator.simulate(
        ...
        rtol=1e-7,
        atol=1e-8,
        stop_on_fuze=parar_simulacao_no_fuze,
        fuze_target_center=alvo["centro"],
        fuze_radius_m=raio_fuze_m,
        fuze_arm_time_s=tempo_armar_fuze_s,
    )

    # -------------------------------------------------------------------------
    # APENDICES: FUZE + DANO
    # -------------------------------------------------------------------------
    burst, dano = avaliar_fuze_e_dano(
        resultado=result,
        alvo=alvo,
        ogiva=ogiva,
        raio_fuze_m=raio_fuze_m,
        tempo_armar_s=tempo_armar_fuze_s,
    )

    print("\n" + "-" * 80)
    print("RESUMO DA TRAJETORIA")
    print("-" * 80)
    print(f"Tempo de voo simulado : {result.tempo_voo:.3f} s")
    print(f"Alcance final         : {result.alcance_max:.3f} m")
    print(f"Altura maxima         : {result.altura_max:.3f} m")
    print(f"Desvio lateral maximo : {result.desvio_lateral_max:.3f} m")
    print(f"Velocidade final      : {result.V_mag[-1]:.3f} m/s")
    print(f"Motivo da parada      : {result.stop_reason}")

    if burst is None or dano is None:
    ...
    print("-" * 80)
    print(f"Fuze acionado dentro do raio : {burst['acionado']}")
    print(f"Origem do ponto de burst     : {burst['origem']}")
    print(f"Indice da amostra de burst   : {burst['idx']}")
    print(f"Tempo do burst               : {burst['tempo_s']:.3f} s")
    print(f"Posicao do burst             : {formatar_vetor(burst['posicao_m'])} m")
    print(f"Distancia burst-alvo         : {dano['distancia_m']:.3f} m")
    print(f"Phi alvo (eixo i -> alvo)    : {dano['phi_alvo_deg']:.3f} deg")
    print(f"Alpha1 zona centro estatica  : {dano['alpha1_zona_centro_deg']:.3f} deg")
    print(f"Phi usado na velocidade      : {dano['phi_velocidade_deg']:.3f} deg")
    print(f"Angulo velocidade -> alvo    : {dano['angulo_vel_fragmento_deg']:.3f} deg")
    print(f"Zona polar NPG estatica      : {dano['zona_polar_npg']}")
    zona_din = dano["zona_polar_dinamica_deg"]
    hits_zona = dano["zona_polar_npg"][2] if dano["zona_polar_npg"] is not None else "n/a"
    print(f"Zona polar dinamica alpha2   : ({zona_din[0]:.3f}, {zona_din[1]:.3f}) deg | hits NPG={hits_zona}")
    print(f"Obliquidade no alvo          : {dano['obliquidade_deg']:.3f} deg")
    print(f"Face dominante do alvo       : {dano['face_alvo_dominante']}")
    print(f"Area exposta do alvo         : {dano['area_exposta_m2']:.6f} m^2")
    print(f"Contribuicoes de area        : {dano['contribuicoes_area_m2']}")
    print(f"Velocidade inicial frag.     : {dano['v_frag_m_s']:.3f} m/s")
    print(f"Fragmentos totais no modelo  : {dano['n_fragmentos_modelo']:.0f}")
    print(f"Fragmentos na zona dinamica  : {dano['n_fragmentos_zona']:.6f}")
    print(f"Area faixa dinamica          : {dano['area_faixa_dinamica_m2']:.6f} m^2")
    print(f"Densidade rho''              : {dano['densidade_efetiva']:.9f} frag/m^2")
    print(f"M fragmentos esperados       : {dano['fragmentos_esperados_area']:.6f}")
    ...
    print(f"Prob. destruicao Bernoulli   : {dano['p_destruicao']:.6%}")

    return result, burst, dano
